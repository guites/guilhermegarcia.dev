...
function combinationTree(suggestions, offset, letters_str) {
  ...
  for (let j = i; j < suggestions.length; j++) {
    ...
    for (let k = j; k < suggestions.length; k++) {
      ...
      for (let l = k; l < suggestions.length; l++) {
        ...
        let secondNode = firstNode.getDescendantByName(second);
        ...
        let thirdNode = secondNode.getDescendantByName(third);
        ...
        let fourthNode = thirdNode.getDescendantByName(fourth);
...
if (process.argv.length < 6) {
  ...
  process.exit(1);
  // input = ["show", "clown", "juggle", "trapeze"];
} else {
